    ...
    class Program
    {
        ...
        public static int MaxArea(int[] height)
        {
            ...
            for (int i = 0; i < height.Length; i++)
            {
                ...
                {
                    var key1 = string.Concat(i, ',', j);
                    var key2 = string.Concat(j, ',', i);
                    if (cacheResult.ContainsKey(key1) || cacheResult.ContainsKey(key2))
                        result = cacheResult[key1];
                    else
                    {
                        result = CalculateArea(Math.Min(height[i], height[j]), Math.Abs(j - i));
                        cacheResult.Add(key1, result);
                        if (!cacheResult.ContainsKey(key2))
                        {
                            cacheResult.Add(key2, result);
                        }
                    }
        ...
        private static int CalculateArea(int height, int distance)
        {
